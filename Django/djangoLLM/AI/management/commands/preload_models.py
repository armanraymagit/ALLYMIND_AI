...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            for model in models:
                ...
                if not any(model in name for name in model_names):
                    ...
                    if "llama3.2" in model or ":" in model:
                        ...
                        try:
                            ...
                            self.stdout.write(
                                self.style.ERROR(f"✗ Failed to pull model {model}: {e}")
                            )
                            continue
                    else:
                        self.stdout.write(
                        ...
                        modelfile_path = (
                            "Modelfile" if "llama" in model else f"Modelfile.{model}"
                        )
                        ...
                        if os.path.exists(modelfile_path):
                            try:
                                ...
                                self.stdout.write(
                                    self.style.SUCCESS(
                                        f"✓ Model {model} created successfully"
                                    )
                                )
                            except Exception as e:
                                self.stdout.write(
                                    self.style.ERROR(
                                        f"✗ Failed to create model {model}: {e}"
                                    )
                                )
                                continue
                        else:
                            self.stdout.write(
                                self.style.ERROR(
                                    f"Model {model} not found and no Modelfile. Skipping."
                                )
                            )
                            continue
                ...
                try:
                    ...
                    response = client.generate(
                        ...
                        options={
                            "num_predict": 1
                        },  # Generate only 1 token to minimize time
                    )
                    ...
                    self.stdout.write(
                        self.style.ERROR(f"✗ Failed to load model {model}: {str(e)}")
                    )
            ...
            self.stdout.write(
                self.style.ERROR(f"Failed to connect to Ollama: {str(e)}")
            )
